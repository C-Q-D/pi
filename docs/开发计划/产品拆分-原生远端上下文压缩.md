...
- 类型：product-scope
- 修订版本：2
- 状态：done（V2-only；`fixture-verified experimental`，真实 R1 与 Codex Legacy 未执行）
...
- 状态：done（OpenAI Public 与 Codex V2；Legacy Unsupported 证据门禁仍 blocked）
...
- V2-only 代码、Session、UI/RPC/Extension、离线组合回归和中文文档均已完成；默认策略仍为 Local。
- 下一步只能由用户选择是否授权 [原子计划中的真实用途 R1 Gate](原子开发计划-原生远端上下文压缩.md#真实用途-r1-gate不属于未授权自动-atom)。R1 会使用用户明确选择的真实认证与 Endpoint，必须重新说明请求数、费用、正式状态和回退方式。
- Codex Legacy 仍缺少可授权第二次认证请求的官方或真实受控 Unsupported Error Shape 证据，不得以模拟 Fixture 解除门禁。
...
| 2 | 按 V2-only 离线交付收口 UNIT-01 至 UNIT-05，保留 Legacy 与真实 R1 门禁 | 所有实现 Atom、388 项离线定向测试、中文文档与隐私审计完成，但真实 Provider 行为仍未获授权验证 |
