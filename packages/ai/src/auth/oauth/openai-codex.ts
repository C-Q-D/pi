...
import { parseOpenAICodexAccountId } from "./openai-codex-jwt.ts";
...
function credentialsFromToken(token: OAuthToken): OAuthCredential {
	const accountId = parseOpenAICodexAccountId(token.access);
	if (!accountId) {
...
export const openaiCodexOAuth: OAuthAuth = {
	...
	async getStableSubject(credential) {
		const tokenSubject = parseOpenAICodexAccountId(credential.access);
		if (!tokenSubject) return undefined;
		return credential.accountId === undefined || credential.accountId === tokenSubject ? tokenSubject : undefined;
	},
